...
if UNDERSAMPLE:
  ...
  min_samples = min(
      ...
      all_dfs[(all_dfs["target"] == 0) & (all_dfs["country"] == "australia")].shape[0],
      all_dfs[(all_dfs["target"] == 1) & (all_dfs["country"] == "australia")].shape[0],
  )

  # undersample
  sampled_dfs = [
    all_dfs[(all_dfs["target"] == 0) & (all_dfs["country"] == "africa")].sample(min_samples, random_state=42),
    all_dfs[(all_dfs["target"] == 1) & (all_dfs["country"] == "africa")].sample(min_samples, random_state=42),
    all_dfs[(all_dfs["target"] == 0) & (all_dfs["country"] == "australia")].sample(min_samples, random_state=42),
    all_dfs[(all_dfs["target"] == 1) & (all_dfs["country"] == "australia")].sample(min_samples, random_state=42),
  ]
...
df_train, df_temp = train_test_split(df_final, test_size=0.4, random_state=42, stratify=df_final["target"])
